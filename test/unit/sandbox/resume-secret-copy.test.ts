/**
 * R16 — resume --copy must apply the same secret-file confirmation and auth
 * tagging as create --copy.
 */
...
describe('resume_sandbox secret --copy gate (R16)', () => {
    install_isolated_home_hooks('patchlab-resume-secret-copy-');

    const patchlab_id = 'pl-resume-secret-copy';
    ...
    beforeEach(() => {
        ...
        repository = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'patchlab-resume-secret-repo-')));
        initialize_repository_with_initial_commit(repository);
        create_patchlab_branch(repository, patchlab_id);

        secret_source = path.join(repository, '.env');
    ...
    it('rejects secret --copy without confirmation in non-interactive resume', async () => {
        ...
        await expect(resume_sandbox(patchlab_id, {
            ...
            prompter: null,
        })).rejects.toThrow('Secret-file --copy not confirmed; resume aborted.');
